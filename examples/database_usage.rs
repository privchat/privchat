...
use privchat_server::repository::{
    channel_repo::ChannelRepository, message_repo::MessageRepository, DatabaseManager,
    PaginationParams,
};
use sqlx::PgPool;
use uuid::Uuid;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let group_channel =
        DbChannel::new(DbChannelType::Group, Some("测试群聊".to_string()), user1_id);

    let created_group = channel_repo.create_channel(&group_channel).await?;

    println!("Created group channel: {:?}", created_group);

    // 添加群成员
    let participant1 =
        DbChannelParticipant::new(created_group.channel_id, user1_id, DbParticipantRole::Owner);
...
async fn channel_message_example(pool: PgPool) -> Result<(), DatabaseError> {
    ...
    let created_message = channel_repo
        .create_channel_message(&channel_message)
        .await?;
...
async fn advanced_query_example(pool: PgPool) -> Result<(), DatabaseError> {
    ...
    let active_channels = channel_repo.get_active_channels(&pagination).await?;
