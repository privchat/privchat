...
- [ ] **消息置顶** - 群组和私聊消息置顶
- [ ] **消息转发优化** - 批量转发、转发链追踪
- [x] **消息搜索** - 全文搜索（`message/history/search`）；高级过滤待做

#### 5. 媒体处理
- [x] **图片处理** - SDK 已实现：图片缩略图生成（默认 0.25）、发送前可选压缩；服务端存储支持 S3/OSS
- [ ] **语音消息** - 语音转文字、播放进度同步
- [x] **视频处理** - SDK 已实现：视频钩子（Thumbnail/Compress）、收到消息后自动下载缩略图；服务端存储支持多存储源
...
**计划任务** (P1):
1. **媒体处理**
   - ✅ 图片压缩和缩略图（SDK 已实现：图片缩略图、视频钩子、收到消息后自动下载缩略图）
   - 语音消息处理
   - 视频压缩（SDK 视频钩子已支持，服务端存储已支持多存储源）
   - 文件预览
