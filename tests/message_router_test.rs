...
use privchat_server::infra::{
    MessageRouter, MessageRouterConfig, SimpleSessionManager, MockMessageSender,
...
async fn test_device_online_offline() {
    ...
    // 注册设备离线
    router.register_device_offline(&user_id, &device_id).await.unwrap();
...
async fn test_online_message_routing() {
    ...
    let result = router.route_message_to_user(&user_id, message).await.unwrap();
...
async fn test_specific_device_routing() {
    ...
    let result = router.route_message_to_device(&user_id, &device_id, message).await.unwrap();
    ...
    let result = router.route_message_to_device(&user_id, &"non_existent_device".to_string(), message).await.unwrap();
...
async fn test_batch_message_routing() {
    ...
    let results = router.route_message_to_users(&user_ids, message).await.unwrap();
...
async fn test_offline_message_delivery() {
    ...
    // 投递离线消息
    let delivered_count = router.deliver_offline_messages(&user_id, &device_id).await.unwrap();
...
async fn test_batch_offline_message_delivery() {
    ...
    // 发送多条离线消息
    for i in 0..5 {
        let message = create_test_message("batch_offline_sender", &format!("Offline message {}", i));
        let result = router.route_message_to_user(&user_id, message).await.unwrap();
        assert_eq!(result.offline_count, 1);
    }
    ...
    // 批量投递离线消息
    let delivered_count = router.deliver_offline_messages_batch(&user_id, &device_id, 3).await.unwrap();
...
async fn test_statistics() {
    ...
    for i in 0..5 {
        ...
        let _result = router.route_message_to_user(&user_id, message).await.unwrap();
    }
    ...
    for user_id in user_ids {
        ...
        let handle = tokio::spawn(async move {
            ...
            for j in 0..messages_per_user {
                let message = create_test_message("concurrent_sender", &format!("Concurrent message {}", j));
                let result = router.route_message_to_user(&user_id, message).await.unwrap();
