...
use privchat_protocol::rpc::sync::{
    BatchGetChannelPtsRequest, BatchGetChannelPtsResponse, ChannelPtsInfo, ClientSubmitRequest,
...
impl SyncService {
    ...
    pub async fn handle_client_submit(
        ...
        let server_pts = match self.cache.get_pts_from_cache(req.channel_id).await? {
            Some(cached_pts) => {
    ...
    pub async fn handle_get_channel_pts(
        ...
        Ok(GetChannelPtsResponse { current_pts })
    }
    ...
    pub async fn handle_batch_get_channel_pts(
        ...
        let mut channel_type_map = std::collections::HashMap::new();

        // 批量查询（优化：可以使用一次数据库查询）
        let channels: Vec<u64> = req
            .channels
            .iter()
            .map(|c| {
                channel_type_map.insert(c.channel_id, c.channel_type);
                c.channel_id
            })
            .collect();
        ...
        for (channel_id, current_pts) in pts_results {
            channel_pts_list.push(ChannelPtsInfo {
                channel_id,
                channel_type: channel_type_map.get(&channel_id).copied().unwrap_or(0),
                current_pts,
            });
        }
