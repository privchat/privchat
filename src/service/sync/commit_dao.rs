...
impl CommitLogDao {
    ...
    pub async fn save_commit(&self, commit: &ServerCommit) -> Result<()> {
        ...
        .await
        .map_err(|e| {
            error!("保存 Commit 失败: channel_id={}, pts={}, error={}", commit.channel_id, commit.pts, e);
    ...
    pub async fn query_commits(
        ...
        let rows = sqlx::query_as::<_, CommitRow>(
            ...
            WHERE channel_id = $1 AND channel_type = $2 AND pts > $3
            ORDER BY pts ASC
            LIMIT $4
            "#
        )
        ...
        .await
        .map_err(|e| {
            error!("查询 Commits 失败: channel_id={}, last_pts={}, error={}", channel_id, last_pts, e);
            crate::error::ServerError::Database(format!("Failed to query commits: {}", e))
        })?;
        
        let commits: Vec<ServerCommit> = rows.into_iter().map(|row| {
            ServerCommit {
                pts: row.pts as u64,
    ...
    pub async fn batch_query_commits(
        &self,
        queries: Vec<(u64, u8, u64, u32)>, // (channel_id, channel_type, last_pts, limit)
    ) -> Result<Vec<Vec<ServerCommit>>> {
        let mut results = Vec::new();
        
        for (channel_id, channel_type, last_pts, limit) in queries {
            let commits = self.query_commits(channel_id, channel_type, last_pts, limit).await?;
    ...
    pub async fn get_latest_commit(
        ...
        .await
        .map_err(|e| {
            error!("获取最新 Commit 失败: channel_id={}, channel_type={}, error={}", channel_id, channel_type, e);
