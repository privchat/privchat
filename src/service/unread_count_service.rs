...
use crate::infra::cache::{CacheManager, UnreadCounts, TwoLevelCache};
...
impl UnreadCountService {
    ...
    /// - count: 增加的数量（默认为 1）
    pub async fn increment(&self, user_id: u64, channel_id: u64, count: u64) -> Result<(), ServerError> {
        // 1. 获取或创建 UnreadCounts
        let mut counts = self.cache_manager.unread_counts
            .get(&user_id)
            .await
        ...
        *counts.counts.entry(channel_id).or_insert(0) += count as i32;
        ...
        // 3. 写回缓存（L1+L2 同时写入）⭐
        self.cache_manager.unread_counts
    ...
    /// 批量增加未读计数
    pub async fn increment_batch(&self, user_id: u64, channel_counts: HashMap<u64, u64>) 
        -> Result<(), ServerError> 
    {
    ...
    /// 批量清空会话未读计数
    pub async fn clear_channels(&self, user_id: u64, channel_ids: &[u64]) -> Result<(), ServerError> {
...
mod tests {
    ...
    async fn test_increment_and_get() {
        ...
        // 增加计数
        service.increment(&user_id, &channel_id, 1).await.expect("增加失败");
        service.increment(&user_id, &channel_id, 2).await.expect("增加失败");
    ...
    async fn test_clear_channel() {
        ...
        // 增加计数
        service.increment(&user_id, &channel1, 5).await.expect("增加失败");
        service.increment(&user_id, &channel2, 10).await.expect("增加失败");
        
        // 清空一个会话
        service.clear_channel(&user_id, &channel1).await.expect("清空失败");
