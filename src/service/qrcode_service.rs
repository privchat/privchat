...
use crate::model::{QRKeyRecord, QRKeyOptions, QRType, QRScanLog};
...
impl QRCodeService {
    ...
    pub async fn generate(
        ...
        // 3. 计算过期时间
        let expire_at = options.expire_seconds.map(|s| Utc::now() + Duration::seconds(s));
        ...
        let record = QRKeyRecord {
            ...
            created_at: Utc::now(),
            expire_at,
            max_usage: if options.one_time { Some(1) } else { options.max_usage },
            used_count: 0,
            revoked: false,
    ...
    pub async fn resolve(
        ...
        // 2. 验证状态
        if record.revoked {
            self.log_scan_failure(qr_key, &scanner_id.to_string(), "QR Key 已被撤销").await;
            return Err(ServerError::BadRequest("QR Key 已被撤销".to_string()));
        }
        
        // 3. 验证过期时间
        if record.is_expired() {
            self.log_scan_failure(qr_key, &scanner_id.to_string(), "QR Key 已过期").await;
            return Err(ServerError::BadRequest("QR Key 已过期".to_string()));
        }
        
        // 4. 验证使用次数
        if record.is_usage_exceeded() {
            self.log_scan_failure(qr_key, &scanner_id.to_string(), "QR Key 使用次数已达上限").await;
            return Err(ServerError::BadRequest("QR Key 使用次数已达上限".to_string()));
        }
        ...
        if record.qr_type == QRType::Group {
            if let Some(expected_token) = record.metadata.get("token").and_then(|t| t.as_str()) {
                if token != Some(expected_token) {
                    self.log_scan_failure(qr_key, &scanner_id.to_string(), "Token 验证失败").await;
    ...
    pub async fn refresh(
        ...
        // 1. 撤销旧的 QR Key
        let old_qr_key = self.revoke_by_target(target_id).await
            .unwrap_or_else(|_| String::new());
        
        // 2. 生成新的 QR Key
        let new_record = self.generate(
            qr_type,
            target_id.to_string(),
    ...
    pub async fn revoke_by_target(&self, target_id: &str) -> Result<String, ServerError> {
        ...
        if let Some(qr_key_entry) = self.target_index.get(target_id) {
            ...
            if let Some(mut record) = self.storage.get_mut(&qr_key) {
                ...
                tracing::info!("✅ 撤销目标的 QR Key: target={}, qr_key={}", target_id, qr_key);
    ...
    pub async fn list_by_creator(
        ...
        self.storage
            ...
            .filter(|entry| {
                ...
                (include_revoked || !record.revoked)
            })
...
mod tests {
    ...
    async fn test_generate_and_resolve_group_qrcode() {
        ...
        // 使用正确的 token 解析
        let resolved = service.resolve(&record.qr_key, "bob", Some("abc123")).await.unwrap();
        assert_eq!(resolved.target_id, "group_123");
        
        // 使用错误的 token 应该失败
        let result = service.resolve(&record.qr_key, "charlie", Some("wrong")).await;
    ...
    async fn test_refresh_qrcode() {
        ...
        // 刷新二维码
        let (revoked_key, new_qr_key) = service.refresh("alice", QRType::User, "alice").await.unwrap();
    ...
    async fn test_one_time_qrcode() {
        ...
        // 第一次解析成功
        let _ = service.resolve(&record.qr_key, "alice", None).await.unwrap();
