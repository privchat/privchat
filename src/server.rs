...
use crate::infra::{
    CacheManager, OnlineStatusManager, OnlineStatusStats, database::Database,
};
use crate::repository::{
    UserRepository, PgChannelRepository, PgMessageRepository,
};
use crate::session::SessionManager;
use crate::config::ServerConfig;
use crate::handler::{
    SendMessageHandler, PingMessageHandler, ConnectMessageHandler, 
    SubscribeMessageHandler, DisconnectMessageHandler, RPCMessageHandler,
};
...
impl ChatServer {
    ...
    pub async fn new(config: ServerConfig) -> Result<Self, ServerError> {
        ...
        let database = Database::new(&config.database_url).await
            .map_err(|e| {
                ServerError::Internal(format!("数据库连接失败: {}", e))
            })?;
        ...
        if config.system_message.enabled {
            info!("🤖 系统消息功能已启用（user_id={} 为系统用户，客户端本地化显示）", crate::config::SYSTEM_USER_ID);
            
            // 🔧 确保系统用户存在于数据库中（用于外键约束）
            info!("🔧 检查系统用户是否存在...");
            match user_repository.find_by_id(crate::config::SYSTEM_USER_ID).await {
                Ok(Some(_)) => {
                    info!("✅ 系统用户已存在: user_id={}", crate::config::SYSTEM_USER_ID);
                }
                Ok(None) => {
                    info!("⚠️ 系统用户不存在，正在创建...");
                    // 创建系统用户记录
                    let system_user_def = crate::config::get_system_user(crate::config::SYSTEM_USER_ID)
                    ...
                    match user_repository.create_with_id(&system_user).await {
                        Ok(_) => {
                            info!("✅ 系统用户创建成功: user_id={}, display_name={}", 
                                crate::config::SYSTEM_USER_ID, system_user_def.display_name);
                        }
                        Err(e) => {
        ...
        let online_status_manager = Arc::new(OnlineStatusManager::new(
            config.cache.online_status.clone(),
        ));
        ...
        let jwt_service = Arc::new(crate::auth::JwtService::new(config.jwt_secret.clone(), token_ttl));
        info!("✅ JWT 服务初始化完成");
        ...
        let service_key_manager = Arc::new(crate::auth::ServiceKeyManager::new_master_key(service_master_key));
        ...
        let login_log_repository = Arc::new(crate::repository::LoginLogRepository::new(pool.clone()));
        info!("✅ 登录日志仓库初始化完成");
        ...
        // 4. 创建 Token 撤销服务
        let token_revocation_service = Arc::new(crate::auth::TokenRevocationService::new(device_manager.clone()));
        ...
        // 🔐 6. 创建认证中间件
        let auth_middleware = Arc::new(crate::middleware::AuthMiddleware::new(auth_session_manager.clone()));
        info!("✅ 认证中间件初始化完成");
        ...
        let user_status_cache: Arc<dyn crate::infra::TwoLevelCache<crate::infra::message_router::UserId, crate::infra::message_router::UserOnlineStatus>> = 
            Arc::new(crate::infra::L1L2Cache::local_only(10000, Duration::from_secs(3600)));
        let offline_queue_cache: Arc<dyn crate::infra::TwoLevelCache<crate::infra::message_router::UserId, Vec<crate::infra::message_router::OfflineMessage>>> = 
            Arc::new(crate::infra::L1L2Cache::local_only(10000, Duration::from_secs(3600)));
        info!("✅ 消息路由缓存创建完成");
        ...
        let session_manager_for_router: Arc<dyn crate::infra::message_router::SessionManager> = 
            Arc::new(crate::infra::message_router::SessionManagerAdapter::new(session_manager.clone()));
        ...
        let unread_count_service = Arc::new(crate::service::UnreadCountService::new(cache_for_unread));
        ...
        message_dispatcher.register_handler(MessageType::PingRequest, Box::new(PingMessageHandler::new()));
        
        // 创建好友服务
        let friend_service = Arc::new(crate::service::FriendService::new());
        
        // 创建黑名单服务
        let blacklist_service = Arc::new(crate::service::BlacklistService::new(cache_manager.clone()));
        info!("✅ 黑名单服务初始化完成");
        ...
        file_service.init().await
            .map_err(|e| ServerError::Internal(format!("文件服务初始化失败: {}", e)))?;
        info!("✅ 文件服务初始化完成（存储源数: {}，默认 id: {}）", file_service.source_count(), config.file_default_storage_source_id);
        ...
        let user_device_repo = Arc::new(crate::repository::UserDeviceRepository::new((*pool).clone()));
        ...
        message_dispatcher.register_handler(MessageType::SendMessageRequest, Box::new(SendMessageHandlerWrapper(send_message_handler.clone())));
        
        message_dispatcher.register_handler(MessageType::SubscribeRequest, Box::new(SubscribeMessageHandler::new()));
        message_dispatcher.register_handler(MessageType::DisconnectRequest, Box::new(DisconnectMessageHandler::new(connection_manager.clone())));  // ✨ 新增参数
        message_dispatcher.register_handler(MessageType::RpcRequest, Box::new(RPCMessageHandler::new(auth_middleware.clone())));
        ...
        // 创建在线状态管理器
        info!("🔧 初始化在线状态管理器...");
        let presence_repository = Arc::new(crate::repository::PresenceRepository::new((*pool).clone()));
        ...
        // 创建 RedisClient
        let redis_client = Arc::new(crate::infra::redis::RedisClient::new(&redis_url).await
        ...
        let registry_dao = Arc::new(crate::service::sync::ClientMsgRegistryDao::new(database.clone()));
        ...
        info!("   - Shadow Ban: {}", if security_config.enable_shadow_ban { "启用" } else { "禁用" });
        info!("   - IP 封禁: {}", if security_config.enable_ip_ban { "启用" } else { "禁用" });
        
        let security_service = Arc::new(crate::security::SecurityService::new(security_config));
        let security_middleware = Arc::new(crate::middleware::SecurityMiddleware::new(security_service.clone()));
        info!("✅ 安全系统初始化完成");
    ...
    fn show_config_info(&self) {
        ...
        info!("  - WebSocket 监听地址: {}", self.config.websocket_bind_address);
        ...
        info!("  - 在线状态清理间隔: {}秒", self.config.cache.online_status.cleanup_interval_secs);
        info!("  - 启用协议: {:?}", self.config.enabled_protocols);
        info!("🔐 安全配置:");
        info!("  - 安全模式: {}", self.config.security.mode);
        info!("  - Shadow Ban: {}", if self.config.security.enable_shadow_ban { "启用" } else { "禁用" });
        info!("  - IP 封禁: {}", if self.config.security.enable_ip_ban { "启用" } else { "禁用" });
        info!("  - 用户限流: {} tokens/s (突发: {})", 
    ...
    /// 创建传输层服务器
    async fn create_transport_server(&self) -> Result<Arc<msgtrans::transport::TransportServer>, ServerError> {
        info!("🔧 创建传输层服务器...");
        ...
        let websocket_config = WebSocketServerConfig::new(&self.config.websocket_bind_address.to_string())
                ...
                match event {
                    ...
                    ServerEvent::MessageReceived { session_id, context } => {
                        ...
                        tokio::spawn(async move {
                            ...
                            info!("🎯 消息分发: {} -> biz_type: {} -> MessageType: {:?}", session_id_clone, biz_type, msg_type);
                            ...
                            match message_dispatcher.dispatch(msg_type, request_context).await {
                                Ok(Some(response)) => {
                                    ...
                                    info!("✅ 消息分发器响应已发送: {} (biz_type: {}, MessageType: {:?})", session_id_clone, biz_type, msg_type);
                                }
                                Ok(None) => {
                                    debug!("消息分发器无响应: {} (biz_type: {}, MessageType: {:?})", session_id_clone, biz_type, msg_type);
                                }
                                Err(e) => {
    ...
    async fn start_stats_updater(&self) {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                let online_stats = online_status_manager.get_stats();
                let mut stats_guard = stats.write().await;
                stats_guard.active_sessions = online_stats.total_sessions as u64;
                stats_guard.uptime_seconds += 60;
                
                info!("📊 服务器统计: 在线会话={}, 总连接={}, 运行时间={}s", 
    ...
    async fn simulate_heartbeat_updates(&self) {
        ...
        for i in 1..=100 {
            let session_id = format!("session_{}", i);
            self.online_status_manager.simple_update_heartbeat(&session_id);
        }
    ...
    async fn simulate_user_activity(&self) {
        ...
        for round in 1..=3 {
            ...
            for i in 1..=20 {
                ...
                // 模拟缓存写入
                if let Err(e) = self.cache_manager.set_user_profile(user_id, user_profile).await {
                    warn!("缓存写入失败: {}", e);
                }
    ...
    async fn start_http_server(&self) -> Result<(), ServerError> {
        ...
        info!("✅ HTTP 文件服务器已在后台启动（端口 {}）", self.config.http_file_server_port);
        Ok(())
    }
}
