...
impl UserRepository {
        ...
        match row {
            Some(r) => {
                Ok(Some(User::from_db_row(
                    ...
                    r.privacy_settings.unwrap_or(serde_json::Value::Object(serde_json::Map::new())),
                    r.created_at,
                    r.updated_at,
                    r.last_active_at,
                )))
    ...
    pub async fn create(&self, user: &User) -> Result<User, DatabaseError> {
        let (_user_id, username, password_hash, phone, email, display_name, avatar_url, user_type, status, privacy_settings, created_at, updated_at, last_active_at) = user.to_db_values();
        ...
        if exists.is_some() {
            return Err(DatabaseError::DuplicateEntry("Username already exists".to_string()));
        }
        ...
        if let Some(ref email_val) = email {
            ...
            .fetch_optional(self.pool.as_ref())
            .await
            .map_err(|e| DatabaseError::Database(format!("Failed to check email: {}", e)))?;
            
            if exists.is_some() {
                return Err(DatabaseError::DuplicateEntry("Email already exists".to_string()));
            }
        }
        ...
        let result = sqlx::query!(
            ...
            user_type,
            status,
            serde_json::to_value(&privacy_settings).unwrap_or(serde_json::Value::Object(serde_json::Map::new())),
            created_at,
            updated_at,
            last_active_at
        )
    ...
    pub async fn create_with_id(&self, user: &User) -> Result<User, DatabaseError> {
        let (user_id, username, password_hash, phone, email, display_name, avatar_url, user_type, status, privacy_settings, created_at, updated_at, last_active_at) = user.to_db_values();
        
        // 检查 user_id 是否已存在
        let exists: Option<(i32,)> = sqlx::query_as(
            "SELECT 1 FROM privchat_users WHERE user_id = $1"
        )
        .bind(user_id as i64)
        .fetch_optional(self.pool.as_ref())
        .await
        ...
        if exists.is_some() {
            return Err(DatabaseError::DuplicateEntry(format!("User with id {} already exists", user_id)));
        }
    ...
    pub async fn update(&self, user: &User) -> Result<User, DatabaseError> {
        let (user_id, username, password_hash, phone, email, display_name, avatar_url, user_type, status, privacy_settings, _created_at, updated_at, last_active_at) = user.to_db_values();
    ...
    /// 更新用户最后登录时间
    pub async fn update_last_login(&self, user_id: u64, _timestamp: u64) -> Result<(), DatabaseError> {
    ...
    /// 获取所有用户（分页，管理 API）
    pub async fn find_all_paginated(&self, page: u32, page_size: u32) -> Result<(Vec<User>, u32), DatabaseError> {
        ...
        let users: Vec<User> = rows.into_iter().map(|r| {
            User::from_db_row(
                r.user_id,
                r.username,
                r.password_hash,
        ...
        let total_result: (i64,) = sqlx::query_as(
            "SELECT COUNT(*) FROM privchat_users"
        )
        .fetch_one(self.pool.as_ref())
        .await
    ...
    pub async fn exists(&self, user_id: u64) -> Result<bool, DatabaseError> {
        ...
        .fetch_optional(self.pool.as_ref())
        .await
        .map_err(|e| DatabaseError::Database(format!("Failed to check user existence: {}", e)))?;
