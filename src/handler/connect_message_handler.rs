...
use crate::auth::{JwtService, TokenRevocationService, DeviceManager, DeviceManagerDb, SessionVerifyResult};
...
impl MessageHandler for ConnectMessageHandler {
    async fn handle(&self, context: RequestContext) -> Result<Option<Vec<u8>>> {
        ...
        if token.is_empty() {
            ...
            return Err(crate::error::ServerError::Unauthorized("缺少认证 token".to_string()));
        }
        
        // 2. 验证 JWT token
        let claims = match self.jwt_service.verify_token(token) {
            Ok(claims) => {
                info!("✅ ConnectMessageHandler: Token 验证成功，用户: {}", claims.sub);
                claims
            }
            Err(e) => {
        ...
        if self.token_revocation_service.is_revoked(&claims.jti).await {
            warn!("❌ ConnectMessageHandler: Token 已被撤销 (jti: {})", claims.jti);
        ...
        let user_id = user_id_str.parse::<u64>()
            .map_err(|_| crate::error::ServerError::BadRequest(format!("Invalid user_id in token: {}", user_id_str)))?;
        ...
        self.auth_session_manager.bind_session(
        ...
        info!("🔗 ConnectMessageHandler: 用户 {} (设备 {}) 请求连接", user_id, device_id);
        ...
        if let Err(e) = self.device_manager.update_last_active(&device_id).await {
            ...
            if e.to_string().contains("设备不存在") {
                ...
                if let Err(reg_err) = self.device_manager.register_device(device).await {
                    ...
                    info!("✅ ConnectMessageHandler: 设备已自动注册: device_id={}", device_id);
                }
            } else {
        ...
        if let Err(e) = self.session_manager.add_user_session(
            user_id.clone(),
            context.session_id,
            device_info,
        ).await {
            ...
            info!("✅ ConnectMessageHandler: 用户 {} 会话已注册 (会话ID: {})", user_id, context.session_id);
        }
        
        // 6.5. 注册用户在线状态到 MessageRouter
        let session_id_str = format!("session-{}", context.session_id);
        if let Err(e) = self.message_router.register_device_online(
            &user_id,
            &device_id,
            &session_id_str,
            &claims.app_id,
        ...
        if let Some(event_bus) = crate::handler::send_message_handler::get_global_event_bus() {
            ...
            if let Err(e) = event_bus.publish(event) {
                ...
                debug!("✅ UserOnline 事件已发布: user_id={}, device_id={}", user_id, device_id);
            }
        }
        
        // 7. ✨ 记录登录日志（仅首次 token 认证时记录）
        // 这将记录：token 首次使用时间、设备信息、IP、风险评分等
        if let Err(e) = self.record_login_log(
            user_id,
            &device_id,
            &claims,
            &connect_request,
...
impl ConnectMessageHandler {
    ...
    async fn record_login_log(
        ...
        info!("📝 记录登录日志: user={}, device={}, token={}", user_id, device_id, token_jti);
        ...
        info!("✅ 登录日志已记录: log_id={}, user={}, device={}", log.log_id, user_id, device_id);
        
        // 8. TODO: 如果是可疑登录，发送通知
        if log.is_suspicious() {
            warn!("🚨 检测到可疑登录: user={}, device={}, risk_score={}", user_id, device_id, risk_score);
            // TODO: 发送系统通知
