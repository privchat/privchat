...
impl RedisClient {
    ...
    pub async fn new(redis_url: &str) -> Result<Self, crate::error::ServerError> {
        let manager = RedisConnectionManager::new(redis_url.to_string())
            .map_err(|e| crate::error::ServerError::Internal(format!("Failed to create Redis manager: {}", e)))?;
        
        let pool = Pool::builder()
            .max_size(10)  // 最大连接数
            .build(manager)
            .await
            .map_err(|e| crate::error::ServerError::Internal(format!("Failed to create Redis pool: {}", e)))?;
        
        // 测试连接
        {
            let mut conn = pool.get().await
                .map_err(|e| crate::error::ServerError::Internal(format!("Failed to get Redis connection: {}", e)))?;
            
            let _: String = conn.ping().await
                .map_err(|e| crate::error::ServerError::Internal(format!("Redis ping failed: {}", e)))?;
        }
    ...
    /// 从连接池获取连接
    async fn get_conn(&self) -> Result<bb8::PooledConnection<'_, RedisConnectionManager>, crate::error::ServerError> {
        self.pool.get().await
            .map_err(|e| crate::error::ServerError::Internal(format!("Failed to get Redis connection: {}", e)))
    }
    ...
    pub async fn setex(&self, key: &str, seconds: usize, value: &str) -> Result<(), crate::error::ServerError> {
        let mut conn = self.get_conn().await?;
        conn.set_ex::<_, _, ()>(key, value, seconds as u64).await
            .map_err(|e| crate::error::ServerError::Internal(format!("Redis SETEX failed: {}", e)))?;
        Ok(())
    }
    ...
    pub async fn expire(&self, key: &str, seconds: usize) -> Result<(), crate::error::ServerError> {
        ...
        conn.expire::<_, ()>(key, seconds as i64).await
            .map_err(|e| crate::error::ServerError::Internal(format!("Redis EXPIRE failed: {}", e)))?;
        Ok(())
    }
    ...
    pub async fn exists(&self, key: &str) -> Result<bool, crate::error::ServerError> {
        ...
        let result: bool = conn.exists(key).await
            .map_err(|e| crate::error::ServerError::Internal(format!("Redis EXISTS failed: {}", e)))?;
        Ok(result)
    }
    ...
    pub async fn keys(&self, pattern: &str) -> Result<Vec<String>, crate::error::ServerError> {
        ...
        let result: Vec<String> = conn.keys(pattern).await
            .map_err(|e| crate::error::ServerError::Internal(format!("Redis KEYS failed: {}", e)))?;
        Ok(result)
    }
    ...
    pub async fn zrangebyscore(&self, key: &str, min: f64, max: f64, limit: Option<usize>) -> Result<Vec<String>, crate::error::ServerError> {
        let mut conn = self.get_conn().await?;
        ...
        let result: Vec<String> = cmd.query_async(&mut *conn).await
            .map_err(|e| crate::error::ServerError::Internal(format!("Redis ZRANGEBYSCORE failed: {}", e)))?;
        Ok(result)
    }
    
    /// ZREMRANGEBYRANK key start stop
    pub async fn zremrangebyrank(&self, key: &str, start: isize, stop: isize) -> Result<(), crate::error::ServerError> {
        let mut conn = self.get_conn().await?;
        conn.zremrangebyrank::<_, ()>(key, start, stop).await
            .map_err(|e| crate::error::ServerError::Internal(format!("Redis ZREMRANGEBYRANK failed: {}", e)))?;
        Ok(())
    }
    ...
    pub async fn sadd(&self, key: &str, member: &str) -> Result<(), crate::error::ServerError> {
        ...
        conn.sadd::<_, _, ()>(key, member).await
            .map_err(|e| crate::error::ServerError::Internal(format!("Redis SADD failed: {}", e)))?;
        Ok(())
    }
    ...
    pub async fn srem(&self, key: &str, member: &str) -> Result<(), crate::error::ServerError> {
        ...
        conn.srem::<_, _, ()>(key, member).await
            .map_err(|e| crate::error::ServerError::Internal(format!("Redis SREM failed: {}", e)))?;
        Ok(())
    }
    ...
    pub async fn smembers(&self, key: &str) -> Result<Vec<String>, crate::error::ServerError> {
        ...
        let result: Vec<String> = conn.smembers(key).await
            .map_err(|e| crate::error::ServerError::Internal(format!("Redis SMEMBERS failed: {}", e)))?;
        Ok(result)
    }
    ...
    pub async fn publish(&self, channel: &str, message: &str) -> Result<(), crate::error::ServerError> {
        let mut conn = self.get_conn().await?;
        conn.publish::<_, _, ()>(channel, message).await
            .map_err(|e| crate::error::ServerError::Internal(format!("Redis PUBLISH failed: {}", e)))?;
        Ok(())
    }
} 
