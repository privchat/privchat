...
impl PresenceManagerWithDb {
    ...
    pub async fn subscribe(
        ...
        debug!("👁️ User {} subscribed to user {}'s presence", subscriber_id, target_user_id);
    ...
    pub fn unsubscribe(&self, subscriber_id: u64, target_user_id: u64) {
        if let Some(mut subscribers) = self.subscriptions.get_mut(&target_user_id) {
            ...
            debug!("👁️ User {} unsubscribed from user {}'s presence", subscriber_id, target_user_id);
            
            if subscribers.is_empty() {
                drop(subscribers);
                self.subscriptions.remove(&target_user_id);
                debug!("🧹 Removed empty subscription entry for user {}", target_user_id);
            }
        }
    }
    ...
    pub fn unsubscribe_all(&self, user_id: u64) {
        ...
        if removed_count > 0 {
            debug!("🧹 User {} offline: removed {} subscriptions", user_id, removed_count);
        }
    }
    ...
    pub async fn user_online(&self, user_id: u64) -> Result<(), ServerError> {
            ...
            if is_first_time {
                ...
                tokio::spawn(async move {
                    if let Err(e) = db_repo_clone.update_last_seen(user_id, now).await {
                        error!("Failed to update last_seen for first-time user {}: {}", user_id, e);
                    } else {
        ...
        if old_status != OnlineStatus::Online {
            debug!("🟢 User {} is now online (first_time={})", user_id, is_first_time);
    ...
    pub async fn user_offline(&self, user_id: u64) -> Result<(), ServerError> {
        ...
        if let Some(ref db_repo) = self.db_repo {
            if let Err(e) = db_repo.update_last_seen(user_id, now).await {
                error!("Failed to update last_seen on offline for user {}: {}", user_id, e);
            }
        }
    ...
    async fn batch_update_db(&self) -> Result<(), ServerError> {
        ...
        // 收集所有待更新的数据
        let updates: Vec<(u64, i64)> = self.pending_updates
            .iter()
    ...
    fn start_cleanup_task(manager: Arc<Self>) {
        ...
        info!("🧹 Started cleanup task (interval={}s, retention={}days)", 
    ...
    async fn notify_subscribers(&self, user_id: u64, status_info: OnlineStatusInfo) {
        ...
        debug!(
            ...
            subscribers.len(), user_id, status_info.status
        );
        
        // TODO: 实现实际的推送逻辑
        for subscriber_id in subscribers {
            debug!("  → Will notify user {} about user {} status: {:?}",
    ...
    pub fn get_stats(&self) -> PresenceStats {
        let total_subscribers: usize = self.subscriptions
            .iter()
