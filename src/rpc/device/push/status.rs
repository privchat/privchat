...
use privchat_protocol::rpc::device::{
    DevicePushInfo, DevicePushStatusRequest, DevicePushStatusResponse,
};
...
pub async fn handle(body: Value, services: RpcServiceContext, ctx: RpcContext) -> RpcResult<Value> {
    tracing::debug!("RPC device/push/status 请求: {:?}", body);
    ...
    // 4. 转换为响应格式
    let device_infos: Vec<DevicePushInfo> = devices
        .into_iter()
        .map(|device| DevicePushInfo {
