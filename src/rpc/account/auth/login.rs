...
pub async fn handle(body: Value, services: RpcServiceContext, _ctx: crate::rpc::RpcContext) -> RpcResult<Value> {
    ...
    let password_hash = user.password_hash
        .ok_or_else(|| RpcError::unauthorized("此账号未设置密码，请使用外部认证系统登录".to_string()))?;
    ...
    let session_version = match services.device_manager_db
        .get_device_with_version(user_id, device_id)
        .await
        .map_err(|e| RpcError::internal(format!("查询设备失败: {}", e)))?
    {
        Some((_, version)) => {
            tracing::info!("✅ 设备已存在: device_id={}, session_version={}", device_id, version);
            version
        }
        None => {
            // 设备不存在，创建新设备
            tracing::info!("🆕 创建新设备: device_id={}", device_id);
            ...
            tracing::info!("✅ 新设备已注册: device_id={}, session_version=1", device_id);
            1 // 新设备使用版本号 1
        }
    };
