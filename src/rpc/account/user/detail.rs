...
pub async fn handle(body: Value, services: RpcServiceContext, ctx: crate::rpc::RpcContext) -> RpcResult<Value> {
    ...
    let source = match source_str.as_str() {
        "search" => {
            let search_session_id = source_id.parse::<u64>()
                .map_err(|_| RpcError::validation(format!("Invalid search_session_id: {}", source_id)))?;
            UserDetailSource::Search { search_session_id }
        },
        "group" => {
            let group_id = source_id.parse::<u64>()
                .map_err(|_| RpcError::validation(format!("Invalid group_id: {}", source_id)))?;
            UserDetailSource::Group { group_id }
        },
        "friend" => {
            let friend_id = source_id.parse::<u64>()
                .map_err(|_| RpcError::validation(format!("Invalid friend_id: {}", source_id)))?;
            UserDetailSource::Friend { friend_id: Some(friend_id) }
        },
        "card_share" => {
            let share_id = source_id.parse::<u64>()
                .map_err(|_| RpcError::validation(format!("Invalid share_id: {}", source_id)))?;
            UserDetailSource::CardShare { share_id }
        },
        _ => {
            return Err(RpcError::validation(
                format!("Invalid source type: {}. Must be one of: search, group, friend, card_share", source_str)
            ));
        }
    };
    
    // 验证访问权限
    match services.privacy_service.validate_detail_access(searcher_id, user_id, source).await {
        Ok(_) => {
            // 权限验证通过，从数据库读取用户资料
            match helpers::get_user_profile_with_fallback(user_id, &services.user_repository, &services.cache_manager).await {
                ...
                Ok(None) => {
                    Err(RpcError::not_found(format!("User '{}' not found", user_id)))
                }
                Err(e) => {
