...
pub async fn request_upload_token(
    ...
    info!(
        ...
        user_id, file_type_str, file_size, business_type
    );
    ...
    let token = services.upload_token_service
        .generate_token(user_id, file_type, max_size, business_type, filename)
        .await
        .map_err(|e| RpcError::internal(e.to_string()))?;
    
    // 构建上传 URL（使用配置的文件服务 API 基础 URL）
    let upload_url = services.config.file_api_base_url
        .as_ref()
        .map(|base_url| {
            format!("{}/files/upload", base_url.trim_end_matches('/'))
        })
        .unwrap_or_else(|| {
            // 如果没有配置，使用默认值（仅用于开发环境）
            format!("http://localhost:{}/api/app/files/upload", services.config.http_file_server_port)
        });
