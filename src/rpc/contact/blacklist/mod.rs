...
pub async fn register_routes(services: RpcServiceContext) {
    ...
    // ✨ 使用路由常量代替硬编码字符串
    router.register(routes::blacklist::ADD, {
    ...
    router.register(routes::blacklist::REMOVE, {
        let services = services.clone();
        Box::new(move |body, ctx| {
            let services = services.clone();
    ...
    router.register(routes::blacklist::LIST, {
        let services = services.clone();
        Box::new(move |body, ctx| {
            let services = services.clone();
    ...
    router.register(routes::blacklist::CHECK, {
        let services = services.clone();
        Box::new(move |body, ctx| {
            let services = services.clone();
