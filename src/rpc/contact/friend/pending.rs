...
pub async fn handle(body: Value, services: RpcServiceContext, ctx: crate::rpc::RpcContext) -> RpcResult<Value> {
    tracing::info!("🔧 处理待处理好友申请列表请求: {:?}", body);
    ...
    match services.friend_service.get_pending_requests(user_id).await {
        Ok(requests) => {
            ...
            for request in requests {
                ...
                if let Some(source) = &request.source {
                    let (source_type, source_id, source_desc) = match source {
                        crate::model::privacy::FriendRequestSource::Search { search_session_id } => {
                            ("search", search_session_id.to_string(), "通过搜索添加".to_string())
                        }
                        crate::model::privacy::FriendRequestSource::Group { group_id } => {
                            // 尝试获取群组名称
                            let group_name = match services.channel_service.get_channel(&group_id).await {
                                Ok(ch) => ch.metadata.name.unwrap_or_else(|| group_id.to_string()),
                                Err(_) => group_id.to_string(),
                            };
                            ("group", group_id.to_string(), format!("通过群组「{}」添加", group_name))
                        }
                        crate::model::privacy::FriendRequestSource::CardShare { share_id } => {
                            // 尝试获取分享者信息（从数据库读取）
                            let sharer_name = match services.cache_manager.get_card_share(*share_id).await {
                                Ok(Some(share)) => {
                                    match helpers::get_user_profile_with_fallback(share.sharer_id, &services.user_repository, &services.cache_manager).await {
                            ...
                            ("card_share", share_id.to_string(), format!("通过{}分享的名片添加", sharer_name))
                        }
                ...
                // 获取申请者的详细信息（从数据库读取）
                if let Ok(Some(profile)) = helpers::get_user_profile_with_fallback(request.from_user_id, &services.user_repository, &services.cache_manager).await {
            ...
            tracing::info!("✅ 获取待处理好友申请列表成功: {} 有 {} 个待处理申请", user_id, request_list.len());
            Ok(json!({
        ...
        Err(e) => {
            ...
            Err(RpcError::internal(format!("获取待处理好友申请列表失败: {}", e)))
        }
    }
}
