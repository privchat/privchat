...
pub async fn handle(body: Value, services: RpcServiceContext, ctx: crate::rpc::RpcContext) -> RpcResult<Value> {
    tracing::info!("🔧 处理 列出 QR 码 请求: {:?}", body);
    
    // 解析参数
    let creator_id = body.get("creator_id").and_then(|v| v.as_str())
        .ok_or_else(|| RpcError::validation("creator_id is required".to_string()))?;
    
    let include_revoked = body.get("include_revoked").and_then(|v| v.as_bool()).unwrap_or(false);
    
    // 获取 QR Keys 列表
    let records = services.qrcode_service.list_by_creator(creator_id, include_revoked).await;
    
    // 转换为响应格式
    let qr_keys: Vec<Value> = records.iter().map(|record| {
        json!({
