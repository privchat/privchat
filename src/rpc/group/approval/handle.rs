...
pub async fn handle(body: Value, services: RpcServiceContext, ctx: crate::rpc::RpcContext) -> RpcResult<Value> {
    ...
    let reject_reason = body.get("reject_reason").and_then(|v| v.as_str()).map(|s| s.to_string());
    
    // 1. 验证 action 参数
    if action != "approve" && action != "reject" {
        return Err(RpcError::validation("action must be 'approve' or 'reject'".to_string()));
    }
    
    // 2. 获取加群请求
    let request = services.approval_service.get_request(request_id).await
        .map_err(|e| RpcError::internal(format!("获取请求失败: {}", e)))?
        .ok_or_else(|| RpcError::not_found("加群请求不存在".to_string()))?;
    
    // 3. 获取群组
    let channel = services.channel_service.get_channel(&request.group_id).await
    ...
    if !matches!(operator_member.role, 
        ...
        return Err(RpcError::forbidden("只有群主或管理员可以审批加群申请".to_string()));
    }
    
    // 5. 根据 action 执行审批
    if action == "approve" {
        // 5.1. 同意申请
        let updated_request = services.approval_service.approve_request(
            request_id,
            operator_id
        ).await
        ...
        tracing::info!("✅ 同意加群申请: request_id={}, user_id={}, group_id={}", 
            request_id, request.user_id, request.group_id);
        ...
        // 5.3. 拒绝申请
        let updated_request = services.approval_service.reject_request(
            request_id,
            operator_id,
            reject_reason.clone()
        ).await
        ...
        tracing::info!("✅ 拒绝加群申请: request_id={}, user_id={}, reason={:?}", 
            request_id, request.user_id, reject_reason);
