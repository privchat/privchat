...
pub async fn handle(body: Value, services: RpcServiceContext, ctx: crate::rpc::RpcContext) -> RpcResult<Value> {
    ...
    if !matches!(operator_member.role, crate::model::channel::MemberRole::Owner) {
    ...
    let max_members = settings.get("max_members").and_then(|v| v.as_u64()).map(|v| v as u32);
    let announcement = settings.get("announcement").and_then(|v| v.as_str()).map(|s| s.to_string());
    let description = settings.get("description").and_then(|v| v.as_str()).map(|s| s.to_string());
    ...
    if let Some(desc) = description {
        services.channel_service.update_channel_metadata(
            &group_id,
            None,  // name不变
            Some(desc),
            None,  // avatar_url不变
        ...
        update_count += 1;
        tracing::info!("✅ 更新群描述成功");
    }
    
    // 4.2. 更新群公告
    if let Some(ann) = announcement {
        services.channel_service.set_channel_announcement(
            &group_id,
            Some(ann)
        ).await
            .map_err(|e| RpcError::internal(format!("更新群公告失败: {}", e)))?;
        update_count += 1;
        tracing::info!("✅ 更新群公告成功");
    }
    
    // 4.3. 更新全员禁言
    if let Some(muted) = all_muted {
        services.channel_service.set_channel_all_muted(
            &group_id,
            muted
        ).await
            .map_err(|e| RpcError::internal(format!("更新全员禁言失败: {}", e)))?;
        update_count += 1;
        tracing::info!("✅ 更新全员禁言成功: {}", muted);
    }
    
    // 4.4. 更新加群审批
    if let Some(approval) = join_need_approval {
        services.channel_service.set_channel_join_approval(
            &group_id,
            approval
        ).await
            .map_err(|e| RpcError::internal(format!("更新加群审批失败: {}", e)))?;
        update_count += 1;
        tracing::info!("✅ 更新加群审批成功: {}", approval);
    }
    
    // 4.5. 更新成员邀请权限
    if let Some(invite) = member_can_invite {
        services.channel_service.set_channel_member_invite(
            &group_id,
            invite
        ).await
            .map_err(|e| RpcError::internal(format!("更新成员邀请权限失败: {}", e)))?;
        update_count += 1;
        tracing::info!("✅ 更新成员邀请权限成功: {}", invite);
    }
    
    // 4.6. 更新成员上限
    if let Some(max) = max_members {
        services.channel_service.set_channel_max_members(
            &group_id,
            max
        ).await
            .map_err(|e| RpcError::internal(format!("更新成员上限失败: {}", e)))?;
        update_count += 1;
        tracing::info!("✅ 更新成员上限成功: {}", max);
    }
    
    tracing::info!("✅ 群设置更新成功: group_id={}, 更新项数={}", group_id, update_count);
