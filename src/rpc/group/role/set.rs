...
pub async fn handle(body: Value, services: RpcServiceContext, ctx: crate::rpc::RpcContext) -> RpcResult<Value> {
    tracing::info!("🔧 处理 设置成员角色 请求: {:?}", body);
    ...
    let role = match role_str.as_str() {
        "admin" => 1,
        "member" => 0,
        _ => return Err(RpcError::validation(
            format!("Invalid role: {}, expected 'admin' or 'member'", role_str)
        )),
    };
    ...
    match services.channel_service.set_member_role(
        ...
        Ok(()) => {
            tracing::info!("✅ 成功设置成员角色: group={}, user={}, role={}", 
                group_id, user_id, role_str);
            Ok(json!({
                "success": true,
                "group_id": group_id,
                "user_id": user_id,
        ...
        Err(e) => {
            tracing::error!("❌ 设置成员角色失败: group={}, user={}, error={}", 
                group_id, user_id, e);
