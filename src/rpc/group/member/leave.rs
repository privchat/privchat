...
pub async fn handle(body: Value, services: RpcServiceContext, ctx: crate::rpc::RpcContext) -> RpcResult<Value> {
    ...
    if !channel.members.contains_key(&user_id) {
        return Err(RpcError::not_found("User is not a member of this group".to_string()));
    }
    ...
    if let Some(member) = channel.members.get(&user_id) {
        if matches!(member.role, crate::model::channel::MemberRole::Owner) {
            return Err(RpcError::forbidden("Group owner cannot leave group. Please transfer ownership or disband the group".to_string()));
        }
    }
    
    // 离开群组
    match services.channel_service.leave_channel(group_id, user_id).await {
        Ok(_) => {
            tracing::info!("✅ 用户 {} 成功退出群组 {}", user_id, group_id);
            // 简单操作，返回 true
            Ok(json!(true))
        }
        Err(e) => {
