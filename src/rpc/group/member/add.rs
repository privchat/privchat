...
pub async fn handle(body: Value, services: RpcServiceContext, ctx: crate::rpc::RpcContext) -> RpcResult<Value> {
    ...
    tracing::info!("🔧 添加群组成员: {} 邀请 {} 加入群组 {} (角色: {})", inviter_id, user_id, group_id, role);
    ...
    if let Err(e) = services.channel_service.add_participant(group_id, user_id, member_role).await {
        tracing::warn!("⚠️ 添加参与者到数据库失败: {}，继续添加到频道", e);
    }
    
    // 添加成员到群组频道（内存）
    match services.channel_service.add_member_to_group(group_id, user_id).await {
        Ok(_) => {
            tracing::info!("✅ 成功添加成员 {} 到群组 {}", user_id, group_id);
            ...
            if member_role != crate::model::channel::MemberRole::Member {
                match services.channel_service.set_member_role(
                    &group_id,
                    &user_id,
                    channel_role,
                ).await {
                    Ok(_) => {
                        tracing::info!("✅ 成功设置成员 {} 角色为 {:?}", user_id, role);
                    }
                    Err(e) => {
                        tracing::warn!("⚠️ 设置成员 {} 角色失败: {}，成员已添加但角色为普通成员", user_id, e);
                    }
                }
            }
