...
pub async fn handle(body: Value, services: RpcServiceContext, ctx: crate::rpc::RpcContext) -> RpcResult<Value> {
    ...
    if !matches!(operator_member.role, crate::model::channel::MemberRole::Owner | crate::model::channel::MemberRole::Admin) {
        return Err(RpcError::forbidden("Only owner or admin can remove members".to_string()));
    }
    ...
    match services.channel_service.leave_channel(group_id, user_id).await {
        Ok(_) => {
            tracing::info!("✅ 成功移除成员 {} 从群组 {}", user_id, group_id);
            // 简单操作，返回 true
            Ok(json!(true))
        }
        Err(e) => {
