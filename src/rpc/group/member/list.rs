...
pub async fn handle(body: Value, services: RpcServiceContext, ctx: crate::rpc::RpcContext) -> RpcResult<Value> {
    ...
    // 获取群成员列表
    match services.channel_service.get_channel_members(&group_id).await {
        Ok(members) => {
            // 获取成员详细信息（从数据库读取）
            let mut member_list = Vec::new();
            for member in members {
                if let Ok(Some(profile)) = helpers::get_user_profile_with_fallback(member.user_id, &services.user_repository, &services.cache_manager).await {
            ...
            tracing::info!("✅ 获取群成员列表成功: {} 有 {} 个成员", group_id, member_list.len());
            Ok(json!({
                "members": member_list,
