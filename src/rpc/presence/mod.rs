...
pub async fn register_routes(services: RpcServiceContext) {
    ...
    GLOBAL_RPC_ROUTER.register(routes::presence::SUBSCRIBE, {
    ...
    GLOBAL_RPC_ROUTER.register(routes::presence::UNSUBSCRIBE, {
        let services = services.clone();
        move |params, ctx| {
            let services = services.clone();
    ...
    GLOBAL_RPC_ROUTER.register(routes::presence::TYPING, {
        let services = services.clone();
        move |params, ctx| {
            let services = services.clone();
    ...
    GLOBAL_RPC_ROUTER.register(routes::presence::STATUS_GET, {
        let services = services.clone();
        move |params, ctx| {
            let services = services.clone();
    ...
    tracing::info!("🔔 Presence 系统路由注册完成 (subscribe, unsubscribe, typing, status)");
}
