...
pub async fn handle(body: Value, services: RpcServiceContext, ctx: crate::rpc::RpcContext) -> RpcResult<Value> {
    ...
    match MessageRepository::list_by_channel(services.message_repository.as_ref(), channel_id, limit, before_created_at).await {
        Ok(messages) => {
            let mut message_list: Vec<Value> = messages.into_iter().map(|msg| {
            ...
            tracing::info!("✅ 从数据库获取到 {} 条历史消息（已按时间升序返回）", message_list.len());
