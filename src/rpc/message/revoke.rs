...
pub async fn handle(body: Value, services: RpcServiceContext, ctx: crate::rpc::RpcContext) -> RpcResult<Value> {
    ...
    info!("🔧 处理消息撤回请求: server_message_id={}, channel_id={}, user_id={}", 
          message_id, channel_id, user_id);
    
    // 1. 从数据库查找消息
    let message = services.message_repository.as_ref().find_by_id(message_id).await
    ...
    let is_admin = if let Some(channel) = services.channel_service.get_channel(&channel_id).await.ok() {
        if let Some(member) = channel.members.get(&user_id) {
            matches!(member.role, crate::model::channel::MemberRole::Owner | crate::model::channel::MemberRole::Admin)
        } else {
            false
        }
    } else {
    ...
    if !is_sender && !is_admin {
        return Err(RpcError::forbidden("只有发送者或群管理员可以撤回消息".to_string()));
    }
    ...
    if is_sender && !is_admin {
        ...
        if now - message_time > DEFAULT_REVOKE_TIME_LIMIT {
            return Err(RpcError::validation(
                format!("消息发送已超过 {} 小时，无法撤回", DEFAULT_REVOKE_TIME_LIMIT / 3600)
            ));
        }
    }
    
    // 5. 在数据库中标记消息为已撤回
    services.message_repository.as_ref().revoke_message(message_id, user_id).await
        .map_err(|e| RpcError::internal(format!("撤回消息失败: {}", e)))?;
    
    info!("✅ 消息已在数据库中标记为撤回: message_id={}, revoked_by={}", message_id, user_id);
    ...
    if let Some(event_bus) = crate::handler::send_message_handler::get_global_event_bus() {
        ...
        if let Err(e) = event_bus.publish(event) {
            ...
            info!("✅ MessageRevoked 事件已发布: message_id={}", message_id);
        }
    }
    
    // 6. 同时更新内存缓存
    if let Err(e) = services.message_history_service.revoke_message(&channel_id, &message_id).await {
        warn!("⚠️ 更新内存缓存失败: {}", e);
    }
    ...
    let participants = services.channel_service.get_channel_participants(channel_id).await
        .map_err(|e| RpcError::internal(format!("获取会话参与者失败: {}", e)))?;
    
    for participant in participants {
        if let Err(e) = services.offline_queue_service.remove_message_by_id(participant.user_id, message_id).await {
            warn!("⚠️ 从用户 {} 的离线队列中删除消息 {} 失败: {}", participant.user_id, message_id, e);
        } else {
            info!("🗑️ 成功从用户 {} 的离线队列中删除消息 {}", participant.user_id, message_id);
        }
    }
    
    info!("✅ 消息撤回完成: message_id={}", message_id);
...
async fn distribute_revoke_event(
    ...
    // 推送给所有参与者
    for participant in participants {
        if let Err(e) = services.message_router.route_message_to_user(
            &participant.user_id,
            revoke_event.clone(),
        ).await {
            warn!("⚠️ 推送撤回事件给用户 {} 失败: {}", participant.user_id, e);
        } else {
            info!("📤 成功推送撤回事件给用户 {}", participant.user_id);
        }
    }
