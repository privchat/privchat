...
pub async fn handle(body: Value, services: RpcServiceContext, ctx: crate::rpc::RpcContext) -> RpcResult<Value> {
    ...
    match services.channel_service.mark_as_read(&user_id, &channel_id, message_id).await {
        Ok(_) => {
            debug!("✅ 会话已读状态已更新: {} 在频道 {} 消息 {}", user_id, channel_id, message_id);
        }
        Err(e) => {
            warn!("⚠️ 更新会话已读状态失败: {}", e);
    ...
    let receipt = match services.read_receipt_service.record_read_receipt(
        message_id,
        channel_id,
        user_id,
    ).await {
        Ok(receipt) => {
            info!("✅ 消息已读回执已记录: {} 在频道 {} 消息 {} (已读时间: {})", 
                  user_id, channel_id, message_id, receipt.read_at);
            receipt
        }
        Err(e) => {
            error!("❌ 记录已读回执失败: {}", e);
    ...
    // 3. ✅ 新增：广播已读通知给发送者
    if let Err(e) = broadcast_read_receipt(
        &services,
        message_id,
        channel_id,
        user_id,
...
async fn broadcast_read_receipt(
    ...
    let message = match services.message_history_service
        ...
        .await
    {
        ...
        Err(e) => {
            ...
            debug!("消息不存在，跳过已读通知广播: message_id={}, error={}", message_id, e);
            return Ok(()); // 优雅返回，不报错
        }
    };
    ...
    // 2. 获取频道信息（判断频道类型）
    // 优化：如果频道不存在，优雅处理
    let channel = match services.channel_service
        .get_channel(&channel_id)
        .await
    {
        Ok(ch) => ch,
        Err(e) => {
            debug!("频道不存在，跳过已读通知广播: channel_id={}, error={}", channel_id, e);
            return Ok(()); // 优雅返回，不报错
        }
    };
    ...
    match channel_kind {
        ChannelKind::PrivateChat => {
            // 私聊：直接通知发送者
            info!("📨 准备广播已读通知：私聊消息 {} 已被 {} 读取，通知发送者 {}", 
                  message_id, reader_id, message.sender_id);
