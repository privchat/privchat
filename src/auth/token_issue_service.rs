...
impl TokenIssueService {
    ...
    pub async fn issue_token(
        ...
        let device_id = if let Some(ref client_device_id) = request.device_id {
            ...
            Uuid::parse_str(client_device_id).map_err(|_| {
    ...
    fn validate_request(request: &IssueTokenRequest) -> Result<()> {
        ...
        if request.device_info.device_name.is_empty() {
            return Err(ServerError::Validation("device_name 不能为空".to_string()));
        }

        // 验证 ttl (如果提供)
        if let Some(ttl) = request.ttl {
            if ttl < 60 {
                return Err(ServerError::Validation("ttl 不能小于 60 秒".to_string()));
            }

            if ttl > 30 * 24 * 3600 {
                // 最大 30 天
                return Err(ServerError::Validation("ttl 不能超过 30 天".to_string()));
            }
        }
