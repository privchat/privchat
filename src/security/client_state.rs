...
impl ClientState {
    ...
    pub fn can_send_message(&self) -> bool {
        matches!(self, ClientState::Normal | ClientState::Throttled | ClientState::ShadowBanned)
    }
...
impl ClientStateManager {
    ...
    /// 转换状态
    pub fn transition_to(&mut self, new_state: ClientState, reason: ViolationType, duration: Option<Duration>) {
    ...
    pub fn auto_escalate(&mut self, violation: ViolationType) {
        ...
        match (current, violation) {
            ...
            (ClientState::Normal, ViolationType::RateLimit) => {
                self.transition_to(ClientState::Throttled, violation, Some(Duration::from_secs(60)));
            }
            
            // 降速 -> 写禁用
            (ClientState::Throttled, ViolationType::RateLimit) => {
                self.transition_to(ClientState::WriteDisabled, violation, Some(Duration::from_secs(300)));
            }
            
            // 持续违规 -> 影子封禁
            (ClientState::WriteDisabled, _) => {
                self.transition_to(ClientState::ShadowBanned, violation, Some(Duration::from_secs(3600)));
            }
            
            // 协议攻击 -> 直接断开
            (_, ViolationType::ProtocolAttack) => {
                self.transition_to(ClientState::Disconnected, violation, Some(Duration::from_secs(1800)));
            }
            
            // Spam -> 影子封禁
            (_, ViolationType::Spam) => {
                self.transition_to(ClientState::ShadowBanned, violation, Some(Duration::from_secs(7200)));
            }
            
            _ => {
                // 默认：降速
                self.transition_to(ClientState::Throttled, violation, Some(Duration::from_secs(30)));
            }
        }
    }
