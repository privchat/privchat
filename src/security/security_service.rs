...
use super::rate_limiter::{MultiDimensionRateLimiter, RateLimitConfig, FanoutCostCalculator};
...
impl IpProtectionRecord {
    ...
    fn add_violation(&mut self) {
        ...
        // 只保留最近1小时的记录
        self.recent_violations.retain(|t| now.duration_since(*t) < Duration::from_secs(3600));
    }
    
    fn should_temp_ban(&self) -> bool {
        // 5分钟内10次违规
        let five_min_ago = Instant::now() - Duration::from_secs(300);
        let recent_count = self.recent_violations.iter()
...
impl SecurityService {
        ...
        if !allowed {
            if let Some(violation_type) = violation {
                ...
                if self.config.mode == SecurityMode::EnforceLight {
                    ...
                    state_manager.trust_score_mut().record_violation(violation_type);
    ...
    pub async fn check_rpc(
        ...
        // ObserveOnly 模式：只记录
        if self.config.mode == SecurityMode::ObserveOnly {
            debug!("🔍 [ObserveOnly] RPC: user={}, method={}", user_id, rpc_method);
            return SecurityCheckResult::allow(ClientState::Normal);
        }
        ...
        if self.config.mode == SecurityMode::EnforceFull {
            ...
            if is_write_rpc && !current_state.can_write_rpc() {
                if current_state.should_silent_drop() && self.config.enable_shadow_ban {
                    warn!("🚫 [EnforceFull] 用户 {} RPC {} 被 Shadow Ban", user_id, rpc_method);
                    return SecurityCheckResult::shadow_ban();
                }
    ...
    pub async fn manual_ban(
        ...
        warn!("管理员封禁用户 {} (设备: {}), 原因: {:?}", user_id, device_id, reason);
    }
    ...
    pub async fn cleanup_expired_data(&self) {
        ...
        info!("安全服务清理完成：保留 {} 个客户端状态，{} 个 IP 记录",
            client_states.len(), ip_protections.len());
    }
}
...
mod tests {
    ...
    async fn test_security_service() {
        ...
        // 正常消息发送
        let result = service.check_send_message(1001, "device_1", 2001, 5, 100, false).await;
        assert!(result.allowed);
        
        // 大群高频发送（应触发 fan-out 检测）
        let result = service.check_send_message(1001, "device_1", 2001, 500, 100, false).await;
        // 第一次可能允许，但多次后会被限制
    }
